     ...
     <div class="p-5 flex-1">
         ...
         <a class= "flex gap-4 text-gray-400 text-sm "href="{{ route('post.show', ['username' => $post->user->username, 'post' => $post]) }}">
           {{ $post->created_at->format('F j, Y') }}
             <svg xmlns="http://www.w3.org/2000/svg" width="24" height="20" viewBox="0 0 24 24" aria-label="clap">
        ...
        <span class="inline-flex gap-1">{{ $post->claps_count }}</span>
         </a>
     </div>

     <a href="{{ route('post.show', ['username' => $post->user->username, 'post' => $post]) }}" class= "">
         <img class="w-48 h-full rounded-r-lg object-cover max-h-64" src= {{ $post->imageUrl('preview') }}
             alt="" />
     </a>
 </div>
